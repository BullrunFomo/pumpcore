...
import SplitNowTab from "./SplitNowTab";
    ...
    <Dialog open={open} onOpenChange={(v) => !v && handleClose()}>
      ...
      >
        ...
        >
          ...
          {/* Tab switcher */}
          <div
            className="flex mt-3 rounded-lg p-0.5 gap-0.5"
            style={{ background: "rgba(0,0,0,0.35)", border: "1px solid rgba(255,255,255,0.06)" }}
          >
            {(["classic", "splitnow"] as const).map((t) => (
              <button
                key={t}
                onClick={() => setTab(t)}
