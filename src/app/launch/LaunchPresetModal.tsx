...
import { X, Crown, Save, Trash2, SlidersHorizontal } from "lucide-react";
...
interface Props {
  slot: number;
  existing: LaunchPreset | null;
  onClose: () => void;
}

export default function LaunchPresetModal({ slot, existing, onClose }: Props) {
  const wallets = useStore((s) => s.wallets);
  const bundlePresets = useStore((s) => s.bundlePresets);
  const setBundlePresets = useStore((s) => s.setBundlePresets);

  const [name, setName] = useState(existing?.name ?? `Preset ${slot + 1}`);
  ...
  const [jitoTipSol, setJitoTipSol] = useState(existing?.jitoTipSol ?? 0.005);
  ...
  const toggle = (id: string) => {
    setSelectedIds((prev) => {
      ...
      setDevWalletId((dev) => (next.includes(dev) ? dev : next[0] ?? ""));
      return next;
    });
  };
      ...
      >
        ...
        <div className="shrink-0 flex items-center gap-3 px-5 py-4" style={{ borderBottom: SEP, background: "rgba(79,131,255,0.03)" }}>
          ...
          <div className="flex-1 min-w-0">
            <input
              ...
              placeholder={`Preset ${slot + 1}`}
              className="w-full bg-transparent text-sm font-bold text-zinc-100 outline-none placeholder-zinc-600"
            />
            <p className="text-[10px] text-zinc-500 mt-0.5">Slot {slot + 1} · click name to rename</p>
          </div>
          ...
          <div className="px-5 pt-5 pb-4" style={{ borderBottom: SEP }}>
            ...
            <div className="flex gap-3 mb-3">
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                ...
                <input
                  type="number" value={jitoTipSol} step={0.001} min={0}
            ...
            {launchType === "stagger" && (
              <div className="flex items-center gap-3 mt-3">
                ...
                <Slider min={0} max={10000} step={100} value={[staggerDelayMs]} onValueChange={([v]) => setStaggerDelayMs(v)} className="flex-1" />
                <span className="text-xs font-semibold tabular-nums shrink-0" style={{ color: "#4f83ff" }}>{staggerDelayMs}ms</span>
              </div>
            )}
          </div>
                ...
                {wallets.map((w) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <Crown className={`h-3.5 w-3.5 ${isDev ? "text-yellow-400" : "text-zinc-600 hover:text-zinc-300"}`} />
                      </button>
        ...
        <div className="shrink-0 flex items-center gap-3 px-5 py-4" style={{ borderTop: SEP }}>
          {existing && (
            <button
              onClick={remove}
              className="flex items-center gap-1.5 px-3 py-2 rounded text-[11px] font-semibold text-red-500 hover:text-red-400 transition-colors"
          ...
          <button onClick={onClose} className="px-3 py-2 rounded text-[11px] font-semibold text-zinc-500 hover:text-zinc-300 transition-colors">
            Cancel
          </button>
          <button
            onClick={save}
